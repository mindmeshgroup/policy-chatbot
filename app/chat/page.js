...
function ChatContent() {
  ...
  // ── Accent colours: staff = deep maroon, student = La Trobe red ─────────────
  ...
  const t = {
    ...
    // Staff sidebar: deep red background; student: white/dark
    sidebarBg:           isStaff
                           ? (darkMode ? '#3a0000' : '#8B0000')
                           : (darkMode ? '#2d2d2d' : 'white'),
    sidebarItemBg:       isStaff
                           ? (darkMode ? '#4a0a0a' : '#9B1010')
                           : (darkMode ? '#3a3a3a' : 'white'),
    sidebarItemBgSel:    isStaff
                           ? (darkMode ? '#2a0000' : '#6B0000')
                           : (darkMode ? '#4a1a20' : '#fff0f0'),
    sidebarItemBorder:   isStaff
                           ? 'rgba(255,255,255,0.18)'
                           : (darkMode ? '#555' : '#d9d9d9'),
    sidebarItemText:     isStaff ? 'white' : (darkMode ? '#ffffff' : '#1a1a1a'),
    sidebarTitleText:    isStaff ? 'white' : (darkMode ? '#ffffff' : '#1a1a1a'),
    sidebarBorder:       isStaff ? accentDark : accent,

    chatPanelBg:         darkMode ? '#2d2d2d' : (isStaff ? '#fdf5f5' : 'white'),
    bubbleBg:            darkMode ? '#3a3a3a' : 'white',
    bubbleBorder:        darkMode ? '#555'    : '#a0a0a0',
    bubbleText:          darkMode ? '#ffffff' : '#1a1a1a',
    disclaimerBg:        darkMode ? '#3a2e00' : (isStaff ? '#fce8e8' : '#fff8e1'),
    disclaimerBorder:    darkMode ? '#5a4800' : (isStaff ? '#f5b8b8' : '#ffe082'),
    disclaimerText:      darkMode ? '#ffd54f' : (isStaff ? '#6B0000' : '#7a6000'),
    ...
    // Staff FAQ chips: deep red bg + white text; student: light red bg + red text
    faqChipBg:           isStaff
                           ? (darkMode ? '#4a0000' : '#8B0000')
                           : (darkMode ? '#3a1a20' : '#fff0f0'),
    faqChipBorder:       isStaff
                           ? 'rgba(139,0,0,0.5)'
                           : (darkMode ? 'rgba(200,16,46,0.45)' : 'rgba(200,16,46,0.25)'),
    faqChipText:         isStaff ? 'white' : accent,
  ...
  const ThemeToggle = ({ style = {} }) => (
    <button
      ...
      onFocus={e => e.currentTarget.style.boxShadow = `0 0 0 3px ${accent}55`}
    ...
    <div style={{ backgroundColor: t.pageBg, minHeight: '100vh', display: 'flex', flexDirection: 'column', fontFamily: "'DM Sans', sans-serif", transition: 'background-color 0.2s' }}>
      ...
      <header className="hidden md:flex" style={{ backgroundColor: t.navBg, padding: '16px 60px', alignItems: 'center', gap: '24px', borderBottom: `1px solid ${t.navBorder}`, position: 'sticky', top: 0, zIndex: 10, transition: 'background-color 0.2s' }}>
        ...
        <div style={{ flex: 1, margin: '0 16px' }}>
          <div style={{ border: `3px solid ${accent}`, borderRadius: '8px', padding: '14px 24px', fontSize: '18px', fontWeight: '700', color: t.navText, display: 'flex', alignItems: 'center', gap: '12px' }}>
            Home
            {isStaff && (
              <span style={{ fontSize: '11px', fontWeight: '700', color: 'white', backgroundColor: accentDark, borderRadius: '6px', padding: '3px 10px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                Staff Portal
              </span>
            )}
          </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
          <ThemeToggle />
          <div
            style={{ width: '64px', height: '64px', borderRadius: '50%', backgroundColor: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', boxShadow: '0 4px 8px rgba(0,0,0,0.25)' }}
            role="img"
      ...
      <header className="flex md:hidden" style={{ backgroundColor: t.navBg, padding: '16px 20px', flexDirection: 'column', alignItems: 'center', borderBottom: `1px solid ${t.navBorder}`, position: 'sticky', top: 0, zIndex: 10, transition: 'background-color 0.2s' }}>
        ...
        <p style={{ fontSize: '13px', fontWeight: '500', color: t.navSubText, margin: '4px 0 4px 0', textAlign: 'center' }}>Policy Chatbot</p>
        {isStaff && (
          <span style={{ fontSize: '10px', fontWeight: '700', color: 'white', backgroundColor: accentDark, borderRadius: '6px', padding: '2px 8px', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '6px' }}>
            Staff Portal
          </span>
        )}
        <div style={{ height: '3px', backgroundColor: accent, width: '70%', borderRadius: '2px' }} />
      </header>
      ...
      <main style={{ display: 'flex', flex: 1, width: '100%', maxWidth: '1440px', margin: '0 auto', padding: '24px 60px', gap: '24px', paddingBottom: '100px' }}>
        ...
        <nav className="hidden md:flex" aria-label="Chat history" style={{ width: '315px', flexShrink: 0, backgroundColor: t.sidebarBg, borderRadius: '12px', border: `2.5px solid ${t.sidebarBorder}`, padding: '24px 20px', flexDirection: 'column', gap: '16px', height: 'fit-content', transition: 'background-color 0.2s' }}>
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ fontSize: '18px', fontWeight: '700', color: t.sidebarTitleText, margin: '0 0 8px 0', fontFamily: "'DM Sans', sans-serif" }}>Chat History</h2>
            <div style={{ height: '2px', backgroundColor: isStaff ? 'rgba(255,255,255,0.4)' : accent, width: '60%', margin: '0 auto' }} />
          </div>
            ...
            {chatHistoryItems.map(item => (
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer', boxShadow: '0 2px 4px rgba(0,0,0,0.12)',
                  transition: 'all 0.2s', fontFamily: "'DM Sans', sans-serif",
                  outline: 'none',
                }}
                onFocus={e => e.currentTarget.style.boxShadow = `0 0 0 3px ${accent}55`}
                onBlur={e => e.currentTarget.style.boxShadow = '0 2px 4px rgba(0,0,0,0.12)'}
        ...
        >
          ...
          >
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: msg.sender === 'user' ? 'flex-end' : 'flex-start', gap: '10px' }}>
                  {msg.sender === 'bot' && (
                    <div
                      ...
                      style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: accentDark, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 1px 2px rgba(0,0,0,0.25)' }}
                    >
                      ...
                      {msg.sources.map((source, si) => (
                        <div key={si}>
                          <a
                            ...
                            onFocus={e => e.currentTarget.style.boxShadow = `0 0 0 3px ${accent}66`}
                            onBlur={e => e.currentTarget.style.boxShadow = 'none'}
                          >
            ...
            {loading && (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }} role="status" aria-label="Chatbot is typing">
                <div style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: accentDark, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} aria-hidden="true">
                ...
                <div style={{ backgroundColor: t.loadingBubbleBg, border: `2px solid ${t.loadingBubbleBorder}`, borderRadius: '8px', padding: '14px 18px', display: 'flex', gap: '5px', alignItems: 'center' }} aria-hidden="true">
                  {[0, 150, 300].map((delay, i) => (
                    <span key={i} className="animate-bounce" style={{ width: '8px', height: '8px', backgroundColor: accentDark, borderRadius: '50%', display: 'inline-block', animationDelay: `${delay}ms` }} />
                  ))}
                </div>
              </div>
            )}
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }} role="group" aria-labelledby="faq-label">
                {faqChips.map((question, i) => (
                  <button
                    ...
                    style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '7px 14px', fontSize: '12px', fontWeight: '500', color: t.faqChipText, backgroundColor: t.faqChipBg, border: `1.5px solid ${t.faqChipBorder}`, borderRadius: '20px', cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.15s', outline: 'none' }}
                    onMouseOver={e => { e.currentTarget.style.backgroundColor = isStaff ? accentHover : (darkMode ? '#5a1a25' : '#ffe0e5'); e.currentTarget.style.borderColor = accent; }}
                    onMouseOut={e => { e.currentTarget.style.backgroundColor = t.faqChipBg; e.currentTarget.style.borderColor = t.faqChipBorder; }}
                    onFocus={e => e.currentTarget.style.boxShadow = `0 0 0 3px ${accent}44`}
                    onBlur={e => e.currentTarget.style.boxShadow = 'none'}
                  >
          ...
          <div style={{ padding: '16px 24px', borderTop: `1px solid ${t.inputWrapBorder}`, backgroundColor: t.inputWrapBg, transition: 'background-color 0.2s' }}>
            <div style={{ display: 'flex', alignItems: 'center', backgroundColor: t.inputInnerBg, border: inputFocused ? `2px solid ${accent}` : `2px solid ${t.inputInnerBorder}`, borderRadius: '8px', overflow: 'hidden', transition: 'border-color 0.2s, background-color 0.2s' }}>
              ...
              {inputValue.length > 400 && (
                <span id="char-count" style={{ fontSize: '11px', color: inputValue.length > 480 ? accent : t.sourcesLabel, padding: '0 8px', flexShrink: 0 }} aria-live="polite">
              ...
              <button
                ...
                style={{ width: '60px', height: '56px', backgroundColor: loading || !inputValue.trim() ? (darkMode ? '#444' : '#e0e0e0') : accentDark, border: 'none', cursor: loading || !inputValue.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s', outline: 'none' }}
                onFocus={e => e.currentTarget.style.boxShadow = `0 0 0 3px ${accent}44`}
                onBlur={e => e.currentTarget.style.boxShadow = 'none'}
              >
      ...
      <nav className="flex md:hidden" aria-label="Main navigation" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, backgroundColor: t.mobileFooterBg, borderTop: `1px solid ${t.mobileFooterBorder}`, padding: '8px 0 12px', justifyContent: 'space-around', zIndex: 20, transition: 'background-color 0.2s' }}>
        {navItems.map(item => (
          <button
            ...
            style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', padding: '6px 16px', borderRadius: '10px', backgroundColor: activeNav === item.label ? accentDark : t.mobileNavBtnBg, boxShadow: activeNav === item.label ? '0 4px 8px rgba(0,0,0,0.2)' : 'none', transition: 'all 0.2s', outline: 'none' }}
            onFocus={e => e.currentTarget.style.boxShadow = `0 0 0 3px ${accent}44`}
